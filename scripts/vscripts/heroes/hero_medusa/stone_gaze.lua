...
function StoneGazeSlow( keys )
	...
	if scepter then
		modifier_target = keys.modifier_target_scepter
	end
...
function StoneGaze( keys )
	...
	local scepter = caster:HasScepter()
	if scepter then
		modifier_slow = keys.modifier_slow_scepter
	end
...
function StoneGazeFacing( keys )
	...
	local scepter = caster:HasScepter()
	if scepter then
		face_duration = ability:GetLevelSpecialValueFor("face_duration_scepter", ability_level)
		stone_duration = ability:GetLevelSpecialValueFor("stone_duration_scepter", ability_level)
	end
