...
function AmplifyDamage( keys )
	local caster = keys.caster
	local target = keys.target
	local ability = keys.ability
	local armor_stacks = math.floor(target:GetPhysicalArmorBaseValue())
	AddStacks(ability, caster, target, keys.damage_scepter, armor_stacks, false)
end

function AmplifyDamage_End( keys )
local target = keys.target

target:RemoveModifierByName("modifier_amplify_damage_scepter")
end