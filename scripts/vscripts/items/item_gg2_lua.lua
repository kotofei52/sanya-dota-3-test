if modifier_gg2_lua == nil then
    modifier_gg2_lua = class({})
end
...
function modifier_gg2_lua:IsBuff()
    return 1
end

function modifier_gg2_lua:OnCreated()
...
function modifier_gg2_lua:OnIntervalThink()
    if IsServer() then
        -- Variables
...
function modifier_gg2_lua:OnRefresh()
    self.inner_vitality_heal = self:GetAbility():GetSpecialValueFor( "heal" )
    self.inner_vitality_attrib_bonus = self:GetAbility():GetSpecialValueFor( "attrib_bonus" )
    self.inner_vitality_hurt_attrib_bonus = self:GetAbility():GetSpecialValueFor( "hurt_attrib_bonus" )
    self.inner_vitality_hurt_percent = self:GetAbility():GetSpecialValueFor("hurt_percent")
...
function modifier_gg2_lua:DeclareFunctions()
    local funcs = {
...
function modifier_gg2_lua:GetModifierConstantHealthRegen( params)
