...
function gg3think( keys )
	local caster = keys.caster
	local target = keys.target
	local ability = keys.ability
	local health_perc = target:GetHealthPercent()

	if (health_perc > 30) then
		target:RemoveModifierByName("modifier_item_gg3_aura_bonus1")
	else
		if not target:HasModifier("modifier_item_gg3_aura_bonus1") then
			ability:ApplyDataDrivenModifier(caster, target, "modifier_item_gg3_aura_bonus1", {})
		else return 1 end
	end
end